    ...
    def choose_action(self, electricity_price, storage_soc):
        if electricity_price == self.min_electricity_price:
            if self.charge_flag == 0 and storage_soc < self.min_charging_storage_soc:
                action = 2  # Charge the storage at nominal mass flow rate
                self.charge_flag = 1
            elif self.charge_flag == 1 and storage_soc < self.max_storage_soc:
                action = 2  # Keep charging the storage at nominal mass flow rate
                self.charge_flag = 1
            elif self.charge_flag == 1 and storage_soc >= self.max_storage_soc:
                action = 1  # the storage is full. stop the charging phase
                self.charge_flag = 0
            else:
                action = 1  # the storage is full. stop the charging phase
                self.charge_flag = 0
